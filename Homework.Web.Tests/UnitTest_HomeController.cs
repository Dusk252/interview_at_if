...
{
    [Fact]
    public async Task Index_ReturnsViewOfExpectedType()
    {
        // Arrange
        ...
        var mockClient = mockHandler.CreateClient();
            mockHandler.SetupRequest(HttpMethod.Get)
                .ReturnsAsync(new HttpResponseMessage {
                    StatusCode = HttpStatusCode.OK,
                    Content = new StringContent(test)
                });
        var controller = new HomeController(mockLogger, new HttpClient(mockHttpHandler.Object));

        // Act
        var result = await controller.Index();

        //Assert
        var contentResult = Assert.IsType<HomeViewModel>(result);
    }

        [Fact]
    public async Task Index_PopulatesViewWhenPassedWithAValidHttpClient()
    {
